...
class SpatialHeteroModel(nn.Module):
    ...
    def forward(self, z1, z2):
        ...
        with torch.no_grad():
            ...
            # print(w.shape) 6,64
        # print(z1.shape) # 32,1,128,64
        # print(z1.reshape(-1, self.d_model).shape) 4096*64
        ...
        # print(zc1.shape) #4096,6 32*128*6
        
        with torch.no_grad():
            q1 = sinkhorn(zc1.detach())
            q2 = sinkhorn(zc2.detach())
            # print(q1.shape)#4096*6
...
class STEncoder(nn.Module):
    ...
    def forward(self, x0, graph):
        ...
        # print(x0.shape) #32,19,128,2
        in_len = x0.size(1) # x0, nlvc time
        if in_len < self.receptive_field:
            x = F.pad(x0, (0,0,0,0,self.receptive_field-in_len,0))
        else:
            x = x0
        # print(self.receptive_field) 21
        # print(x.shape) #32,21,128,2
        x = x.permute(0, 3, 1, 2)  # (batch_size, feature_dim, input_length, num_nodes), nclv 
        # print(x.shape) #32,2,21,128
        ## ST block 1
        x = self.tconv11(x)    # nclv  32,32,19,128   connect old feature and conv feature
        
        x, x_agg, self.t_sim_mx = self.pooler(x) # t_sim_mx 19,32,128
            #32,32,128
...
class Pooler(nn.Module):
    ...
    def __init__(self, n_query, d_model, agg='avg'):
        """
        :param n_query: number of query 17
        :param d_model: dimension of model 64
        """
    ...
    def forward(self, x):
        ...
        # print(x.shape) #32,32,19,128
        # print(self.n_query) 17
        x_in = self.align(x)[:, :, -self.n_query:, :] # ncqv  input and outp align
        # print(x_in.shape)# 32,32,17,128

        # calculate the attention matrix A using key x     
        A = self.att(x) # x: nclv, A: nqlv  FCLayer
        # print(A.shape) 32,17,19,128
        A = F.softmax(A, dim=2) # nqlv  normalize the time dimension
        # print(A.shape) 32,17,19,128

        # calculate region embeding using attention matrix A
        x = torch.einsum('nclv,nqlv->ncqv', x, A) # 32,32,17,128
        x_agg = self.agg(x).squeeze(2) # ncqv->ncv #32,32,128 AVGPooling
        x_agg = torch.einsum('ncv->nvc', x_agg) # ncv->nvc 32,128,32

        # calculate the temporal simlarity (prob)
        A = torch.einsum('nqlv->lnqv', A)  #19,32,17,128
        A = self.softmax(self.agg(A).squeeze(2)) # A: lnqv->lnv 19,32,128 AVGPooling

        # print(torch.relu(x + x_in).shape) # 32,32,17,128
        # print(x_agg.detach().shape) #32,128,32
        # print(A.detach().shape) # 19,32,128
